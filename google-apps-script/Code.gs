...
function doPost(e) {
  try {
    ...
    const rows=Math.max(sheet.getLastRow()-1,1); sheet.getRange(2,1,rows,1).setNumberFormat('dd/MM/yyyy'); sheet.getRange(2,6,rows,1).setNumberFormat('0.00'); sheet.getRange(2,24,rows,7).setNumberFormat('€ #,##0.00'); sheet.getRange(2,36,rows,1).setNumberFormat('dd/MM/yyyy HH:mm:ss'); sheet.autoResizeColumns(1,HEADERS.length);
