...
function doGet(e) {
  try {
    ...
    const seller = e.parameter.seller || 'all';
    const center = e.parameter.center || 'all';
    ...
    ss.getSheets().forEach(sheet => {
      ...
      values.slice(1).forEach(row => {
        const dateValue = row[index['Data']];
        if (!dateValue) return;
        const isoDate = Utilities.formatDate(new Date(dateValue), Session.getScriptTimeZone(), 'yyyy-MM-dd');
        const rowSeller = String(row[index['Venditore']] || '');
        const rowCenter = String(row[index['Centro di competenza']] || '');
        if (isoDate < from || isoDate > to) return;
        if (seller !== 'all' && rowSeller !== seller) return;
        if (center !== 'all' && rowCenter !== center) return;
        rows.push({
          date: isoDate,
          seller: rowSeller,
          center: rowCenter,
          callsMade: cellNumber_(row,index,'Telefonate fatte'),
          callsAnswered: cellNumber_(row,index,'Telefonate risposte'),
          appointmentsFromCalls: cellNumber_(row,index,'Appuntamenti da telefonate'),
          quotesFromCalls: cellNumber_(row,index,'Preventivi da telefonate'),
          subscriptionsFromCalls: cellNumber_(row,index,'Abbonamenti da telefonate'),
          toursDone: cellNumber_(row,index,'Tour fatti'),
          quotesFromTours: cellNumber_(row,index,'Preventivi da tour fatti'),
          subscriptionsFromTours: cellNumber_(row,index,'Abbonamenti da tour'),
          organicQuotes: cellNumber_(row,index,'Preventivi organici'),
          subscriptionsFromOrganicQuotes: cellNumber_(row,index,'Abbonamenti da preventivi organici'),
          organicRenewals: cellNumber_(row,index,'Rinnovi organici'),
          trialsActivated: cellNumber_(row,index,'Prove attivate'),
          passesVouchersDelivered: cellNumber_(row,index,'Pass / Voucher Consegnati'),
          passesVouchersActivated: cellNumber_(row,index,'Pass / Voucher Attivati'),
          revenue: cellNumber_(row,index,'Fatturato'),
          futuraAmount: cellNumber_(row,index,'Futura'),
          totalCollected: cellNumber_(row,index,'Totale Incassato'),
          collectedPos: cellNumber_(row,index,'Inc. POS'),
          collectedCash: cellNumber_(row,index,'Inc. Contanti'),
          collectedBank: cellNumber_(row,index,'Inc. Bonifico'),
          collectedFinance: cellNumber_(row,index,'Inc. Finanziamento')
        });
      });
    });
...
function cellNumber_(row,index,header) {
