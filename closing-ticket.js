(()=>{
  ...
  section.innerHTML=`
    ...
    <div id="eveningClosingFields" class="grid grid-2" hidden>
      ...
      <div class="field money-field"><label for="ticketAmount">Ticket</label><div class="currency-wrap"><span>€</span><input id="ticketAmount" name="ticket" type="number" min="0" step=".01" value="0"></div></div>
    </div>`;
  ...
  style.textContent=`
    .card-evening-closing{background:#f8fafc;border:1px solid #cbd5e1}
  ...
  renderSummary=function(target,payload){
    ...
    target.insertAdjacentHTML('beforeend',`<section class="summary-section" data-summary-box="closing"><h3>CHIUSURA SERALE - FUTURA E TICKET</h3><div class="summary-grid"><div class="summary-item"><span>Incassato Futura</span><strong>${formatMoney(payload.futuraAmount)}</strong></div><div class="summary-item"><span>Ticket</span><strong>${formatMoney(payload.ticket)}</strong></div></div></section>`);
  };
