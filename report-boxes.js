(() => {
  ...
  function renderTotalsBoxes(rows){const{a,collected,quotesTotal,installmentsTotal,notesFilled}=totalsData(rows);container.innerHTML=[
    ...
    box('01','Azioni da telefonate e tour',item('Telefonate fatte',formatNumber(a.callsMade))+item('Telefonate risposte',formatNumber(a.callsAnswered))+item('Appuntamenti',formatNumber(a.appointmentsFromCalls))+item('Tour fatti',formatNumber(a.toursDone))),
    box('04','Altre azioni',item('Pass/Voucher attivati',formatNumber(a.passesVouchersActivated))+item('Pass/Voucher consegnati',formatNumber(a.passesVouchersDelivered))),
  ...
  function exportDetailedJpg(){
    const rows=typeof currentRows!=='undefined'?currentRows:[];const{a,collected,quotesTotal,installmentsTotal,notesFilled}=totalsData(rows);
    const sections=[
      ...
      {title:'Azioni da telefonate e tour',color:'#eef6ff',values:[['Telefonate fatte',formatNumber(a.callsMade)],['Telefonate risposte',formatNumber(a.callsAnswered)],['Appuntamenti',formatNumber(a.appointmentsFromCalls)],['Tour fatti',formatNumber(a.toursDone)]]},
      {title:'Altre azioni',color:'#f7f1ff',values:[['Pass/Voucher attivati',formatNumber(a.passesVouchersActivated)],['Pass/Voucher consegnati',formatNumber(a.passesVouchersDelivered)]]},
    ...
    const gap=18,colW=(w-110-gap)/2,cardH=190;sections.forEach((section,i)=>{const col=i%2,row=Math.floor(i/2),x=55+col*(colW+gap),y=265+row*(cardH+gap);roundedRect(ctx,x,y,colW,cardH,16,section.color);ctx.fillStyle='#6d28d9';ctx.font='700 22px Arial';ctx.fillText(section.title,x+20,y+34);const valueCols=section.values.length>3?3:Math.max(section.values.length,1),itemW=(colW-40-(valueCols-1)*10)/valueCols;section.values.forEach((entry,idx)=>{const r=Math.floor(idx/valueCols),c=idx%valueCols,ix=x+20+c*(itemW+10),iy=y+55+r*62;ctx.fillStyle='rgba(255,255,255,.82)';ctx.fillRect(ix,iy,itemW,52);ctx.fillStyle='#64748b';ctx.font='14px Arial';ctx.fillText(entry[0].slice(0,25),ix+10,iy+18);ctx.fillStyle='#111827';ctx.font='700 20px Arial';ctx.fillText(String(entry[1]),ix+10,iy+42)})});
    const chartY=1360,sellerVals=sellers.map(s=>{const sellerRows=rows.filter(r=>r.seller===s);return{seller:s,revenue:sellerRows.reduce((sum,r)=>sum+n(r.revenue),0),collected:sellerRows.reduce((sum,r)=>sum+rowCollected(r),0)}});ctx.fillStyle='#111827';ctx.font='700 28px Arial';ctx.fillText('Fatturato e incassato per venditore',55,chartY);ctx.fillStyle='#6d28d9';ctx.fillRect(55,chartY+20,22,12);ctx.fillStyle='#475569';ctx.font='17px Arial';ctx.fillText('Fatturato',86,chartY+31);ctx.fillStyle='#0891b2';ctx.fillRect(200,chartY+20,22,12);ctx.fillStyle='#475569';ctx.fillText('Incassato',231,chartY+31);const max=Math.max(...sellerVals.flatMap(x=>[x.revenue,x.collected]),1);sellerVals.forEach((it,i)=>{const y=chartY+65+i*125,revW=900*it.revenue/max,incW=900*it.collected/max;ctx.fillStyle='#374151';ctx.font='700 20px Arial';ctx.fillText(it.seller,55,y+38);ctx.fillStyle='#6d28d9';ctx.fillRect(190,y,Math.max(revW,2),30);ctx.fillStyle='#111827';ctx.font='700 17px Arial';ctx.fillText(formatMoney(it.revenue),200+revW,y+21);ctx.fillStyle='#0891b2';ctx.fillRect(190,y+45,Math.max(incW,2),30);ctx.fillStyle='#111827';ctx.fillText(formatMoney(it.collected),200+incW,y+66)});ctx.fillStyle='#6b7280';ctx.font='17px Arial';ctx.fillText(`Report esportato il ${new Intl.DateTimeFormat('it-IT',{dateStyle:'medium',timeStyle:'short'}).format(new Date())}`,55,h-38);canvas.toBlob(blob=>{if(!blob)return;const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`report-vendite-${els.dateFrom.value}-${els.dateTo.value}.jpg`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)},'image/jpeg',.94)
  }
  setTimeout(()=>{const oldExport=document.getElementById('exportJpg');if(!oldExport)return;const newExport=oldExport.cloneNode(true);oldExport.replaceWith(newExport);newExport.addEventListener('click',()=>{try{exportDetailedJpg()}catch(error){const message=document.getElementById('reportMessage');if(message)message.textContent=`Esportazione non riuscita: ${error.message}`}})},0);
})();